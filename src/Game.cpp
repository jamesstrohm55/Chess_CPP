...
void Game::run() {
    ...
    while (result == GameResult::IN_PROGRESS) {
        ...
        if (input == "help") {
            ...
            std::cout << "Enter moves in UCI format (e.g., e2e4, e7e8q for promotion) or algebraic notation (e.g., Nf3, 0-0)\n";
            continue;
        }
        ...
        if (!move.isValid()) {
            std::cout << "Invalid input. Type 'help' for commands and move formats.\n";
            continue;
        }
...
Move Game::parseUCI(const std::string& input) const {
    if (input.length() < 4 || input.length() > 5) return Move(); //Invalid length

    Square from = Square::fromAlgebraic(input.substr(0, 2));
    Square to = Square::fromAlgebraic(input.substr(2, 2));
    if (!from.isValid() || !to.isValid()) return Move(); //Invalid squares
...
Move Game::parseAlgebraic(const std::string& input) const {
    ...
    if (s == "O-O" || s == "0-0") {
        for (auto& legal : currentLegalMoves) {
            if (legal.isCastling && legal.to.col == 6) { //Kingside
                return legal;
            }
        }
        return Move();
    ...
    if (s == "0-0-0" || s == "O-O-O") {
        for (auto& legal : currentLegalMoves) {
            if (legal.isCastling && legal.to.col == 2) { //Queenside
                return legal;
            }
        }
        return Move();
    ...
    for (auto& legal : currentLegalMoves) {
        ...
        if (p.type != type) continue; //Wrong piece type
        if (legal.to != dest) continue; //Wrong destination
        if (legal.promotionPiece != promo) continue; //Wrong promotion
        if (disambigFile >= 0 && legal.from.col != disambigFile) continue; //File disambiguation doesn't match
        if (disambigRank >= 0 && legal.from.row != disambigRank) continue; //Rank disambiguation doesn't match
